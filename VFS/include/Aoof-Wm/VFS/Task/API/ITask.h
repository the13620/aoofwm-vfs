...
#ifndef __AOOFWM_VFS_TASK_API_ITASK_H__
# define __AOOFWM_VFS_TASK_API_ITASK_H__
		...
		namespace	Task
	    {
	    	namespace	API
			{
				class	ITask
			    {
			       ...
			       * 	some maintainance behaviors on a given filesystem.
			       *
			       * 	For example
			       * 		-backups generation
			       * 		-temporary files cleaning
			       * 		-authentication tokens generation
			       * 		-FileHandles Pool maintainance
			       * 		-FileSystem Refresh / Synchronization
			       * 		 (could be useful for standard filesystems, but
			       * 		  also for control version systems...)
			       * 		-Auto Update System / dynamic providers' loading
			       * 		-
			       * 		-...
			       *
			       */
...
#endif	// __AOOFWM_VFS_TASK_API_ITASK_H__
